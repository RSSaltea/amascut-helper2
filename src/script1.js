/* ============================================================
   Amascut Helper — post-Jan26 UI update hardened version
   - Fixes A1lib vs a1lib casing (prevents silent hard-crash)
   - Makes color matching more tolerant (distance-based)
   - Adds a toggleable debug color dump to re-learn new chat colors
   - Lets you temporarily disable readargs.colors filtering to confirm reads
   ============================================================ */
...
(function () {
  ...
  /* ---------------- a1lib/A1lib compatibility ---------------- */
  const A1 = window.a1lib || window.A1lib || null;
  if (!A1) log("⚠️ a1lib not found (window.a1lib / window.A1lib). Some functions may fail.");

  function mixColor(r, g, b) {
    if (!A1?.mixColor) throw new Error("mixColor missing (a1lib.mixColor)");
  ...
  /* ============================================
     Debug toggles (NEW)
     ============================================ */
  let DEBUG_DUMP_COLORS = (localStorage.getItem("amascut.debugDumpColors") ?? "false") === "true";
  let DISABLE_COLOR_FILTER = (localStorage.getItem("amascut.disableColorFilter") ?? "false") === "true";

  (function injectDebugButtons() {
    const style = document.createElement("style");
    style.textContent = `
      .ah-debug{position:fixed;left:8px;bottom:12px;z-index:11052;display:flex;gap:6px;flex-wrap:wrap}
      .ah-debug button{font-size:11px;opacity:.85;background:#222;border:1px solid #444;border-radius:4px;
        cursor:pointer;padding:4px 8px;line-height:1;color:#fff}
      .ah-debug button:hover{opacity:1}
    `;
    document.head.appendChild(style);

    const wrap = document.createElement("div");
    wrap.className = "ah-debug";

    const b1 = document.createElement("button");
    const b2 = document.createElement("button");

    function refresh() {
      b1.textContent = `🎨 Dump colors: ${DEBUG_DUMP_COLORS ? "On" : "Off"}`;
      b2.textContent = `🚫 Color filter: ${DISABLE_COLOR_FILTER ? "Off" : "On"}`;
    }

    b1.addEventListener("click", () => {
      DEBUG_DUMP_COLORS = !DEBUG_DUMP_COLORS;
      try { localStorage.setItem("amascut.debugDumpColors", String(DEBUG_DUMP_COLORS)); } catch {}
      refresh();
      log(`🎨 Dump colors now ${DEBUG_DUMP_COLORS ? "ON" : "OFF"}`);
    });

    b2.addEventListener("click", () => {
      DISABLE_COLOR_FILTER = !DISABLE_COLOR_FILTER;
      try { localStorage.setItem("amascut.disableColorFilter", String(DISABLE_COLOR_FILTER)); } catch {}
      applyReadArgs(); // reapply
      refresh();
      log(`🚫 Color filter now ${DISABLE_COLOR_FILTER ? "DISABLED" : "ENABLED"}`);
    });
  ...
  function startOverlayPosMode() {
    ...
    const step = () => {
      ...
      const mp =
        (A1 && typeof A1.getMousePosition === "function" && A1.getMousePosition()) ||
        null;
  ...
  function openOptionsPopup() {
    ...
    const win = window.open("", "AmascutOptions", "width=420,height=420,resizable=yes");
    if (!win) {
...
<style>
  body{margin:8px;background:#1b1f24;color:#ddd;font-family:rs-pro-3, system-ui, -apple-system, Segoe UI, Arial, sans-serif;}
  h4{margin:0 0 8px 0;font-size:14px;color:#fff;display:flex;justify-content:space-between;align-items:center;}
  button{font-size:12px;}
  .row{display:flex;align-items:center;gap:8px;margin:6px 0;}
  .row label{font-size:12px;min-width:110px;}
  .row input[type="range"]{flex:1;}
  .small{font-size:11px;opacity:.8;}
  .btn{border:1px solid #444;background:#222;color:#fff;border-radius:4px;padding:4px 8px;cursor:pointer;}
  .btn-row{display:flex;gap:6px;flex-wrap:wrap;margin-top:6px;}
  .voice-grid{margin-top:4px;display:flex;flex-direction:column;gap:2px;max-height:200px;overflow-y:auto;border:1px solid #333;padding:4px;border-radius:4px;}
  .voice-item{display:flex;align-items:center;gap:4px;font-size:11px;}
</style>
</head>
<body>
  <h4>
  ...
  <script>
    (function(){
      ...
      if (!parent) { document.body.innerHTML = "<p>Parent window not available.</p>"; return; }
      ...
      function getState() {
        if (typeof parent.amascutGetState === "function") return parent.amascutGetState();
        return { overlayScale: parent.overlayScale || 1, overlayEnabled: !!parent.overlayEnabled, overlayPos: parent.overlayPos || null, posMode: !!parent.posMode };
      }

      function buildVoiceList() {
        if (!voiceList || voiceList._built) return;
        ...
        Object.keys(labels).forEach(function(key){
          ...
          cb.addEventListener("change", function(){
            if (typeof parent.amascutSetVoiceEnabled === "function") parent.amascutSetVoiceEnabled(key, cb.checked);
          });
        });
      }
      ...
      function refreshFromParent(){
        ...
        if (pos && Number.isFinite(pos.x) && Number.isFinite(pos.y)) posVal.textContent = "Position: (" + pos.x + ", " + pos.y + ")";
      ...
      size.addEventListener("input", function () {
        ...
        if (typeof parent.amascutSetOverlayScale === "function") parent.amascutSetOverlayScale(v);
        else { parent.overlayScale = v; try { parent.localStorage.setItem("amascut.overlayScale", String(v)); } catch(e){} }
        sizeVal.textContent = v.toFixed(2) + "×";
      });

      enableCb.addEventListener("change", function () {
        var on = enableCb.checked;
        if (typeof parent.amascutSetOverlayEnabled === "function") parent.amascutSetOverlayEnabled(on);
        else { parent.overlayEnabled = on; try { parent.localStorage.setItem("amascut.overlayEnabled", String(on)); } catch(e){} if (!on && parent.clearOverlayGroup) parent.clearOverlayGroup(); }
        enableState.textContent = on ? "On" : "Off";
      });

      setPosBtn.addEventListener("click", function () {
        var st = getState();
        if (!st.posMode) { parent.startOverlayPosMode(); setPosBtn.textContent = "Saving… (Alt+1)"; }
        else { parent.stopOverlayPosMode(true); setPosBtn.textContent = "Set pos"; }
      });
  ...
  /* ============================================================
     Chat reader config — FIXED for Jan26 changes
     ============================================================ */

  // You can re-learn these via 🎨 Dump colors toggle.
  // If DISABLE_COLOR_FILTER is ON, these won't matter until you turn filter back on.
  const NAME_RGB = [69, 131, 145];
  const TEXT_RGB = [153, 255, 153];
  const WHITE_RGB = [255, 255, 255];
  const PUB_BLUE = [127, 169, 255];

  // Distance-based tolerance (more resilient to anti-aliasing changes)
  ...
  function isColorNear(rgb, target, tol = 40) {
  ...
  function applyReadArgs() {
    if (DISABLE_COLOR_FILTER) {
      ...
      log("ℹ️ reader.readargs: NO color filter (debug)");
      return;
    }

    // NOTE: Alt1 Chatbox reader uses "colors" as a hint/filter. If these are wrong post-update,
    // you can toggle 🚫 Color filter OFF to confirm reading, then re-learn with 🎨.
    reader.readargs = {
      colors: [
        mixColor(...NAME_RGB),
        mixColor(...TEXT_RGB),
    ...
    log("ℹ️ reader.readargs: WITH color filter");
  }
  ...
  function firstNonWhiteColor(seg) {
    ...
    for (const f of seg.fragments) {
      if (!isNearWhite(f.color)) return f.color;
    }
    return null;
  }
    ...
    try {
      if (!reader.pos || !reader.pos.mainbox?.rect) {
        ...
        if (reader.pos?.mainbox?.rect) {
          ...
          log("✅ Chatbox found at " + JSON.stringify(r));
          try { showSelected(reader.pos); } catch {}
        }
      }
    } catch (e) {
      log("⚠️ ensureChatFound: " + (e?.message || e));
    }
  }
  ...
  function readChatbox() {
    ...
    // Optional debug: dump fragment colors of Amascut lines to update constants.
    if (DEBUG_DUMP_COLORS) {
      let dumped = 0;
      for (const seg of segs) {
        if (dumped > 4) break;
        if (!seg?.fragments?.length || typeof seg.text !== "string") continue;
        if (!/Amascut/i.test(seg.text)) continue;

        const cols = seg.fragments
          .slice(0, 10)
          .map(f => `[${(f.color || []).join(",")}]`)
          .join(" ");
    ...
    for (let i = 0; i < segs.length; i++) {
      ...
      // More tolerant "Amascut name" check:
      // Old: exact NAME_RGB; New: near NAME_RGB *or* just match text contains Amascut and has many fragments.
      const hasNameColor = seg.fragments.some(f => isColorNear(f.color, NAME_RGB, 45));
      const looksLikeAmascut = /Amascut/i.test(seg.text);

      if (!looksLikeAmascut) continue;
      // If you want stricter filtering, require hasNameColor. Post-update, name color may vary so this is safer:
      // if (!hasNameColor) continue;

      let full = seg.text;
      const colon = full.indexOf(":");
      if (colon !== -1) full = full.slice(colon + 1).trim();

      // Join continuation lines that are "body text-ish"
      for (let j = i + 1; j < segs.length; j++) {
        const s2 = segs[j];
        if (!s2.fragments || !s2.fragments.length) break;

        const col = firstNonWhiteColor(s2);
        // Prefer TEXT_RGB-ish lines; tolerance bumped
        if (col && isColorNear(col, TEXT_RGB, 55)) {
          full += " " + s2.text.trim();
        } else {
          break;
        }
  ...
  // Extra safety: keep ensuring chatbox is found (UI move/resize recovery)
  setInterval(ensureChatFound, 2000);

  /* ============================================================
     Overlay controller (unchanged except A1lib fixes)
     ============================================================ */
  const overlayCtl = {
  ...
  // expose positioning funcs for popup
